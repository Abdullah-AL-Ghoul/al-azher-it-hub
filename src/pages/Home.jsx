...
import { getLectures, getSources, getUserStats, getFavorites, getAdditions } from '../services'
import { pageContainer, pageItem, pageContainerReduced } from '../utils/motionTokens'
...
export default function Home() {
 ...
 useEffect(() => {
  document.title = isArabic ? 'الرئيسية - AL-Azher IT Hub' : 'Home - AL-Azher IT Hub'
 }, [isArabic])

 const [lectures, setLectures] = useState([])
 const [sources, setSources] = useState([])
 const [additions, setAdditions] = useState([])
 const [userStats, setUserStats] = useState({ viewed: [], lastVisit: null })
 ...
 const continueWatching = useMemo(() => {
  if (!userStats.viewed?.length) return []
  const viewedSet = new Set(userStats.viewed)
  const lastIds = [...userStats.viewed].slice(-4)
  return lastIds
   .map(vid => lectures.find(l => l.id === vid))
   .filter(Boolean)
   .filter(l => viewedSet.has(l.id))
 }, [userStats.viewed, lectures])
 ...
 if (loading) {
   ...
   <div className="min-h-screen pt-24 pb-16 bg-spatial-page">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {[1,2,3,4].map(i => <div key={i} className="glass rounded-xl p-6"><div className="skeleton h-12 w-12 rounded-xl mb-4" /><div className="skeleton h-5 w-3/4 rounded-lg mb-2" /><div className="skeleton h-3 w-1/2 rounded-full" /></div>)}
     </div>
    </div>
   </div>
  )
  ...
  return (
   <motion.div variants={prefersReduced ? pageContainerReduced : containerVariants} initial="hidden" animate="visible" className="grain">
   <HeroSection 
    ctaLink="/lectures" 
    ...
    studentsCount={userStats.viewed?.length || 0}
   />
     ...
     <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="glass rounded-2xl p-6 border border-white/10 bg-gradient-to-l from-royal-500/5 to-cyan-500/5">
       <div className="flex items-center justify-between">
        ...
        <Link
         to="/profile"
         className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl btn-spatial text-navy-900 dark:text-white text-sm font-medium"
        >
     ...
     <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between mb-6">
       <h2 className="text-2xl md:text-3xl font-bold gradient-text-spatial flex items-center gap-2">
        <FiPlay size={24} className="text-royal-500 dark:text-cyan-400" />
        {isArabic ? 'متابعة المشاهدة' : 'Continue watching'}
       </h2>
       <Link to="/lectures" className="text-royal-500 dark:text-cyan-400 hover:text-royal-600 dark:hover:text-cyan-300 font-medium text-sm transition-colors flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
       {continueWatching.map((lecture) => {
        ...
        return (
         <Link key={lecture.id} to={`/lecture/${lecture.id}`} className="group glass glass-hover lift rounded-xl overflow-hidden block">
          <div className="relative h-24 bg-black/30 overflow-hidden">
           {videoId && <img src={lectureThumb(videoId, 'mq')} alt="" width="320" height="180" loading="lazy" decoding="async" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />}
           <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
            <div className="w-9 h-9 bg-rose-500/90 rounded-full flex items-center justify-center text-white shadow-lg shadow-rose-500/30">
             <FiPlay size={14} className="ms-0.5" />
            </div>
           </div>
           <span className="absolute top-2 right-2 px-2 py-0.5 bg-black/50 backdrop-blur-sm rounded-full text-[10px] text-white font-medium">
            {isArabic ? 'أكمل المشاهدة' : 'Resume'}
           </span>
          </div>
          <div className="p-3">
           <p className="text-[10px] text-slate-500 dark:text-white/50 mb-0.5">{isArabic ? lecture.subjectAr : lecture.subjectEn}</p>
           <h3 className="text-xs font-semibold text-navy-900 dark:text-white line-clamp-1 group-hover:text-royal-500 dark:group-hover:text-cyan-300 transition-colors">{isArabic ? lecture.titleAr : lecture.titleEn}</h3>
          </div>
         </Link>
        )
       })}
       ...
       {subjectProgress.map((sp, i) => {
        ...
        return (
         <motion.div
          ...
          className="glass rounded-2xl p-4 border border-white/10 hover:border-emerald-500/30 transition-colors"
         >
          <div className="flex items-center justify-between mb-2">
           <span className="text-sm font-semibold text-navy-900 dark:text-white truncate">{isArabic ? sp.subjectAr : sp.subjectEn}</span>
           <span className="text-xs font-medium text-emerald-500 dark:text-emerald-400 tabular-nums">{pct}%</span>
          </div>
          ...
          <p className="text-xs text-slate-500 dark:text-white/50 mt-2">{isArabic ? `${sp.watched} من ${sp.total} محاضرة` : `${sp.watched} of ${sp.total} lectures`}</p>
         </motion.div>
        )
       })}
      </div>
   ...
   {/* Stats Cards - All Directions Animation */}
   {!isAdmin && (
    <motion.section variants={itemVariants} whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="py-12 bg-spatial-page">
     <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
       {[
        { icon: FiEye, value: userStats.viewed?.length || 0, label: t('home.stats.watched'), gradient: 'from-cyan-500 to-cyan-600', delay: 0, dir: 'right' },
        { icon: FiHeart, value: favorites.length, label: t('home.stats.favorites'), gradient: 'from-rose-500 to-rose-600', delay: 0.08, dir: 'left' },
        { icon: FiFileText, value: lectures.length, label: t('home.stats.totalLectures'), gradient: 'from-violet-500 to-violet-600', delay: 0.16, dir: 'top' },
        { icon: FiLink, value: sources.length, label: isArabic ? 'المصادر' : 'Sources', gradient: 'from-emerald-500 to-emerald-600', delay: 0.24, dir: 'bottom' },
        { icon: FiGrid, value: materialsCount, label: isArabic ? 'عدد المواد' : 'Materials', gradient: 'from-amber-500 to-amber-600', delay: 0.32, dir: 'right' },
       ].map((stat, i) => {
        const Icon = stat.icon
        const dirAnim = {
         right: { opacity: 0, x: 60, y: 0, scale: 0.85 },
         left: { opacity: 0, x: -60, y: 0, scale: 0.85 },
         top: { opacity: 0, x: 0, y: -60, scale: 0.85 },
         bottom: { opacity: 0, x: 0, y: 60, scale: 0.85 },
        }
        return (
         <motion.div
          key={i}
          initial={prefersReduced ? {} : dirAnim[stat.dir]}
          whileInView={prefersReduced ? {} : { opacity: 1, x: 0, y: 0, scale: 1 }}
          viewport={{ once: true }}
         ...
         >
          <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center text-white shadow-lg group-hover:scale-110 group-hover:rotate-3 transition duration-300`}>
           <Icon size={22} />
          </div>
          <div>
           <motion.p
            ...
            viewport={{ once: true }}
            transition={prefersReduced ? {} : { duration: 0.8, delay: stat.delay + 0.3, type: 'spring', stiffness: 120 }}
            className="font-extrabold text-navy-900 dark:text-white text-2xl tracking-tight"
           >
            {stat.value}
           </motion.p>
    ...
    >
     <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
      <div className="flex items-center justify-between">
       ...
       <Link to="/additions" className="text-royal-500 dark:text-cyan-400 hover:text-royal-600 dark:hover:text-cyan-300 font-medium text-sm transition-colors flex items-center gap-1">
        {isArabic ? 'عرض الكل' : 'View All'} <FiArrowLeft className={isArabic ? '' : 'rotate-180'} />
       </Link>
      </div>
     </div>
        ...
        const addDirAnim = {
         ...
         return (
          ...
          >
           ...
           <h4 className="font-semibold text-navy-900 dark:text-white text-sm line-clamp-2 mb-1 group-hover:text-royal-500 dark:group-hover:text-cyan-300 transition-colors">
         ...
         <motion.div key={link.key} variants={itemVariants}>
          <Link to={link.to} className="group spotlight-card lift glass rounded-xl p-6 block cursor-glow hover:border-royal-500/40 transition-colors duration-300" onMouseMove={(e) => { const r = e.currentTarget.getBoundingClientRect(); e.currentTarget.style.setProperty('--mouse-x', `${((e.clientX - r.left) / r.width) * 100}%`); e.currentTarget.style.setProperty('--mouse-y', `${((e.clientY - r.top) / r.height) * 100}%`) }}>
           <div className="flex items-start gap-4">
            ...
            <div>
             <h3 className="font-semibold text-navy-900 dark:text-white group-hover:text-royal-500 dark:group-hover:text-cyan-300 transition-colors mb-1">{t(`home.quickLinks.${link.key}`)}</h3>
   ...
   <motion.section variants={itemVariants} whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="py-16 bg-spatial-page relative section-lazy">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
     <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
      {[
       { value: lectures.length, suffix: '+', label: isArabic ? 'محاضرة' : 'Lectures', gradient: 'from-violet-500 to-cyan-500', dir: 'right' },
       { value: sources.length, suffix: '+', label: isArabic ? 'مصدر' : 'Sources', gradient: 'from-emerald-500 to-cyan-500', dir: 'left' },
       { value: userStats.viewed?.length || 0, suffix: '', label: isArabic ? 'تمت مشاهدتها' : 'Watched', gradient: 'from-amber-500 to-rose-500', dir: 'top' },
       { value: favorites.length, suffix: '', label: isArabic ? 'مفضلة' : 'Favorites', gradient: 'from-rose-500 to-pink-500', dir: 'bottom' },
      ].map((stat, i) => {
       const platDirAnim = {
       ...
       {latestLectures.map((lecture) => {
        ...
        <motion.div key={lecture.id} variants={itemVariants}>
          ...
          <div className="p-4">
           ...
           <h4 className="font-semibold text-navy-900 dark:text-white text-sm mb-1 line-clamp-2 group-hover:text-royal-500 dark:group-hover:text-cyan-300 transition-colors">{isArabic ? lecture.titleAr : lecture.titleEn}</h4>
      ...
      {[
       ...
       return (
        <motion.div key={i} variants={itemVariants} className="group spotlight-card lift glass rounded-2xl p-6 border border-white/10 hover:border-royal-500/30 transition-colors" onMouseMove={(e) => { const r = e.currentTarget.getBoundingClientRect(); e.currentTarget.style.setProperty('--mouse-x', `${((e.clientX - r.left) / r.width) * 100}%`); e.currentTarget.style.setProperty('--mouse-y', `${((e.clientY - r.top) / r.height) * 100}%`) }}>
         ...
         <h3 className="font-semibold text-navy-900 dark:text-white mb-1">{isArabic ? f.titleAr : f.titleEn}</h3>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
     <motion.div variants={itemVariants} className="relative overflow-hidden glass rounded-3xl border border-white/10 p-10 md:p-14 text-center">
      ...
      <div className="relative flex flex-col sm:flex-row gap-3 justify-center">
       ...
       <Link to="/contact" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl glass font-semibold text-[15px] text-navy-900 dark:text-white">
